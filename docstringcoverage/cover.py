...
def get_docstring_coverage(filenames, count_magic=True, count_private=True,
                           skip_empty_files=True, verbose_level=0):
    '''
    ...
    @param count_private: a bool. If False, skips all _private
                        methods and does not include them in the report
    ...
    def printDocstring(base, node):
        ...
        if isDoc == False:
            if not count_magic and name.startswith('__') and name.endswith('__'):
                ...
                docs_needed -= 1
            if not count_private and name.startswith('_'):
                # don't count private methods
                docs_needed -= 1
            else:
        ...
        for symbol in childNodes:
            temp_docs_needed, temp_docs_covered, temp_missing_list = printDocstring(
                '%s.' % name, symbol)
    ...
    for filename in filenames:
        ...
        for symbol in module[-1]:
            ...
            temp_docs_needed, temp_docs_covered, missing_list = printDocstring(
                '', symbol)
    ...
    if(not count_private):
        postfix += " (all private methods omitted!)"

    log("\n", 2)

    if len(filenames) > 1:
        log("Overall statistics for %s files%s:" %
            (len(filenames), postfix), 1)
    else:
    ...
    log("Total docstring coverage: %.1f%%; " %
        (total_result_dict['coverage']), 1, True)
...
def main():
    ...
    parser.add_option("-p", "--noprivate",
                      action="store_false", dest="private", default=True,
                      help="don't count docstrings for _private methods")
            ...
            for fname in fnames:
                if fname.endswith('.py'):
                    ...
                    if(root.endswith(os.sep)):
                        sep = ""  # root can be "./", we should not use separator then
                    filenames.append(root + sep + fname)

    if(len(filenames) < 1):
        sys.exit('No python files found')

    get_docstring_coverage(filenames, options.magic, options.private,
